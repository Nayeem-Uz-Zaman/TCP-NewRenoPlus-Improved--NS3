...
Reads wifi_mobile_results.csv and generates 20 comparison graphs:
  4 varied parameters × 5 metrics
  Each graph shows TcpNewReno vs TcpNewRenoPlusMod on the same axes.

Output: plots/wifi/{metric}_vs_{param}.png  (20 files)

Usage:
    python3 plot_wifi.py [--csv wifi_mobile_results.csv] [--outdir plots/wifi]
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="Plot wifi-mobile-sim comparison graphs")
    p.add_argument("--csv",    default="wifi_mobile_results.csv", help="Input CSV file")
    p.add_argument("--outdir", default="plots/wifi",              help="Output directory for PNGs")
    return p.parse_args()
...
def load_data(csv_path: str) -> pd.DataFrame:
    if not os.path.exists(csv_path):
        ...
        print("Run 'bash run_wifi_sweep.sh' first to generate results.", file=sys.stderr)
        sys.exit(1)
...
def plot_sweep(df: pd.DataFrame, param: str, metric: str, outdir: str):
    ...
    """
    # Build filter mask: all other params at default
    ...
    for variant, style in VARIANT_STYLES.items():
        ...
        if vdata.empty:
            continue
        # Average over duplicate rows (in case sweep ran multiple times)
